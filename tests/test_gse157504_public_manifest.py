...
from scripts.replay_public_dataset_manifest import _safe_argv, replay_file
...
class GSE157504PublicManifestTests(unittest.TestCase):
    ...
    def test_real_manifest_replays_in_isolated_mirror(self):
        manifest = ROOT / "validation" / "public-data" / "GSE157504-candidate-channel-rhythm-provenance-manifest.json"
        result = replay_file(manifest, ROOT, timeout_seconds=180)
        self.assertEqual(result["status"], "verified_public_dataset_replay", result.get("issues"))
