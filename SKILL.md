...
没有实际输入文件时，只能交付方法、模拟示例或信息获取实验；不得把预期输出写成已观察结果。公开数据任务还必须保留 accession、来源 URL、输入/输出文件哈希、脚本版本、供人阅读的命令、结构化 `command_argv` 和分析上下文；读取 `references/public-dataset-manifest.md`。新分析先建立 `planning` 清单并运行 validator 预检，通过后才执行计划中的命令；记录实际输出及哈希后转为 `executed`，再次 validator + 隔离 replay；两者通过后才转为 `verified`，并再次 validator + replay。`planning` 清单不可 replay；不要从 benchmark 的参考答案、README 预期行为或文件命名反推真实生物学结论。
